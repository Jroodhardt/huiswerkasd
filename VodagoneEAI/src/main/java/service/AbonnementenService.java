...
public class AbonnementenService {
    ...
    public Response addAbonnement(@QueryParam("token")String token, SearchAbonnementDTO abonnement) throws SQLException {
        if(tokenDAO.tokenCheck(token)) {
            abonnementenDAO.addAbonnement(abonnement,token);
            Abonnementen newAbonnementen = abonnementenDAO.getAbonnementen(token);
            return Response.ok(newAbonnementen).build();
        }else{
            return Response.status(403).entity("Invalid token").build();
        }
    }
